...
void CollisionModel::MDInteractionsModel::modifyVelocity(Core::Particle& particle, double dt) {
    ...
    double collisionRadius = 1.5*(particle.getDiameter() + collisionGasDiameter_m_)/2.0; 
    ...
    do{
        ...
        }while(circleVectorMagnitude > 1 || directionAngle > 2 * collisionTheta);
...
bool CollisionModel::MDInteractionsModel::leapfrogIntern(std::vector<CollisionModel::Molecule*> moleculesPtr, double dt, double finalTime, double requiredRad){
    // static std::ofstream distanceOut;
    // if(!distanceOut.is_open())
    //     distanceOut.open("distance_output.txt");
        ...
        for(size_t k = 0; k < moleculesPtr_size; ++k){
            for(size_t l = k+1; l < moleculesPtr_size; ++l){
                if((moleculesPtr.at(l)->getComPos() - moleculesPtr.at(k)->getComPos()).magnitude() > startDistances.at(index++)){
                    ...
                    // if(wasHit == false){
                    //     distanceOut << (moleculesPtr.at(l)->getComPos() - moleculesPtr.at(k)->getComPos()) << std::endl;
                    // }
                    wasHit=true;
                }
            }
    ...
    for (size_t i = 0; i+1 < nMolecules; i++){
        for (size_t j = i+1; j < nMolecules; j++){
            ...
            */ 
            ...
            for(auto& atomI : moleculesPtr[i]->getAtoms()){
                for(auto& atomJ : moleculesPtr[j]->getAtoms()){
                    
                    // First contribution: Lennard-Jones potential 
                    // This always contributes to the experienced force 
                    Core::Vector absPosAtomI = moleculesPtr[i]->getComPos() + atomI->getRelativePosition();
                    Core::Vector absPosAtomJ = moleculesPtr[j]->getComPos() + atomJ->getRelativePosition();


                    Core::Vector distance = absPosAtomI - absPosAtomJ;
                    if(distance.magnitude() < 1E-25){
                        forceMolecules[i] += Core::Vector(1e-10, 1e-10, 1e-10);
                        forceMolecules[j] += Core::Vector(1e-10, 1e-10, 1e-10) * (-1);
                        break;
                    }
                    ...
                    forceMolecules[i] += atomForce;
                    forceMolecules[j] += atomForce * (-1);
                    ...
                    if(int(atomI->getCharge()/Core::ELEMENTARY_CHARGE) != 0 && 
                        moleculesPtr[j]->getIsIon() == false &&
                        moleculesPtr[j]->getIsDipole() == false){
                        currentCharge = atomI->getCharge();

                    }else if (moleculesPtr[i]->getIsIon() == false && 
                                int(atomJ->getCharge()/Core::ELEMENTARY_CHARGE) != 0 &&
                                moleculesPtr[i]->getIsDipole() == false){
                    ...
                    currentCharge = 0;
                    if(int(atomI->getCharge()/Core::ELEMENTARY_CHARGE) != 0 && 
                        moleculesPtr[j]->getIsDipole() == true){

                        currentCharge = atomI->getCharge();
                        dipoleX = moleculesPtr[j]->getDipole().x();
                        dipoleY = moleculesPtr[j]->getDipole().y();
                        dipoleZ = moleculesPtr[j]->getDipole().z();
                        dipoleDistanceScalar =  dipoleX * distance.x() + 
                                                dipoleY * distance.y() + 
                                                dipoleZ * distance.z();

                    }else if (moleculesPtr[i]->getIsDipole() == true && 
                                int(atomJ->getCharge()/Core::ELEMENTARY_CHARGE) != 0){

                        currentCharge = atomJ->getCharge();
                        dipoleX = moleculesPtr[i]->getDipole().x();
                        dipoleY = moleculesPtr[i]->getDipole().y();
                        dipoleZ = moleculesPtr[i]->getDipole().z();
                        dipoleDistanceScalar =  dipoleX * distance.x() + 
                                                dipoleY * distance.y() + 
                                                dipoleZ * distance.z();
                    }
                    ...
                    forceMolecules[i] += ionDipoleForce;
                    forceMolecules[j] += ionDipoleForce * (-1);
                }
            }
            ...
            forceMolecules[i] += ionInducedForce;
            forceMolecules[j] += ionInducedForce * (-1);
