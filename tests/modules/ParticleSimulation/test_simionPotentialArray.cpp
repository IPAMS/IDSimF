...
TEST_CASE("SIMION PA basic tests","[SimionPotentialArray]") {
        ...
        SECTION("Test with planar 3d PA with spatial scaling and translation") {
                ...
                REQUIRE(simPaScaled.getInterpolatedPotential(40.0, -5.0, 40.0) == Approx(-8537.619));
        ...
        SECTION("Test with cylindrical PA with synthetic flow data") {
                std::string paFilename = "SIMION_test_flow_axi2d.pa";
                ParticleSimulation::SimionPotentialArray simPa(paFilename);
                simPa.printState();
                CHECK(simPa.getInterpolatedPotential(0.07, 0.06, 0.0) == Approx(-26.2472137093));
                CHECK(simPa.getInterpolatedPotential(0.07, 0.00, 0.0) == Approx(-62.0253164557));

                simPa.setCornerPosition({10.0, -5.0, 4.0});
                CHECK(simPa.getInterpolatedPotential(10.07, -4.94, 4.0) == Approx(-26.2472137093));
                CHECK(simPa.getInterpolatedPotential(10.07, -5.00, 4.0) == Approx(-62.0253164557));
        }
}
